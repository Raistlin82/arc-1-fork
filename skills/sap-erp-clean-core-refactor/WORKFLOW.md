...
| 2 Inventory | `SAPRead(type="DEVC")` recursive, `SAPSearch(searchType="tadir_lookup", names=[...])` for exact-name validation, red-flag `grep` pre-scan | **MCP** ARC-1 | Deterministic |
...
| **Phase 0 — mechanical burn-down** (all rewrite/mechanical objects, one sweep) | Per object: `SAPRead` source → `SAPDiagnose(action="quickfix")` at ATC finding lines → choose matching proposal → `apply_quickfix` deltas → merge → `SAPLint(action="lint_and_fix")` → `SAPLint(action="format")` → `SAPDiagnose(action="syntax", source="<candidate>")` → `SAPWrite(action="update")` → `SAPActivate`; own transport; ATC re-run refreshes plan numbers | **MCP** ARC-1 | **Deterministic — lights-out** (no per-unit review; transport diff review still required) |
...
| | ② rewrite per [`PATTERNS.md`](./PATTERNS.md) 9.1/9.2 recipes → `SAPLint(action="lint_and_fix")` → `SAPLint(action="format")` → `SAPDiagnose(action="syntax", source="<candidate>")` → `SAPWrite` → `SAPActivate` | **CHAIN** + **MCP** ARC-1 | **Generative** — the LLM writes the ABAP; lint, format and syntax are Deterministic pre-write gates |
| | ③ `/abap-cloud-review` — cheap review BEFORE the ATC round-trip | **PLUGIN** `sap-abap` | Generative (review) |
| | ④⑤⑥ `SAPDiagnose(action="atc")` + `unittest` + `SAPRead(type="<type>", name="<name>", action="diff")`; ⑦ rollback from version history (`SAPRead(type="VERSIONS", name="<name>", objectType="<type>")` → `SAPRead(type="VERSION_SOURCE", versionUri="<revision_uri>")` → `SAPWrite`) on regression | **MCP** ARC-1 | Deterministic gates + **Human** confirmation on the diff |
