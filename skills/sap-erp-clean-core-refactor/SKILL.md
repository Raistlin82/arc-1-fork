...
- `sap-fiori-tools` — branch-MUST when the Level A path includes generated Fiori Elements UI; if Fiori MCP/tooling is absent, use the documented local generator/manual fallback and mark the branch degraded
...
- `sapui5-linter` — branch-MUST for UI5/FE frontend quality. Satisfy it with the installed skill/plugin when exposed, or with local `@ui5/linter` / project lint scripts when the skill command is absent
