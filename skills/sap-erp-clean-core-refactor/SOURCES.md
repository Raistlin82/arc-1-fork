...
| `mcp-sap-docs` / `abap_mcp_server` — `sap_discovery_center_service` when a service name/id is known; unified `search(...)` for discovery | `discovery-center` row (SPA scrape) | Prefer the exposed structured service-details tool when available. Do not assume a dedicated Discovery Center search function exists; discover service names through unified search or manual source review |
