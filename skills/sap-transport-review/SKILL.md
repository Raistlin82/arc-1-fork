...
3. Never pass `pgmid` (`LIMU`, `LANG`), a CTS subtype (`METH`, `REPS`), or `wbtype` (`CLAS/OM`,
   `PROG/I`, …) to `SAPRead(type="<type>", name="<name>")`.
...
- **Quality**: when the user requests `+atc`, or risk justifies it, run `SAPDiagnose(action="atc", type="<type>", name="<name>")`
  only for the bounded changed set. Use `SAPLint(action="lint", source="<source>", name="<name>")` for a cheaper local pass.
