...
3. Add migration signals:
   ...
   - Dependencies/where-used: `SAPContext(action="deps", type="<type>", name="<name>")` or `SAPNavigate(action="references", type="<type>", name="<name>", maxResults=1000)`
...
Default to `SAPRead(type="TABLE_QUERY", name="<table>")` — `data` scope, structured `where`, no user-written SQL.
...
discovering objects by type. Do this instead — `SAPRead(type="DEVC", name="<package>")` for each package in scope and
...
One gotcha: `SAPRead(type="DEVC", name="<package>")` omits legacy SEGW types, so use TADIR for those (or
...
|---|---|---|
| Eventing via RAP (AMQP/MQTT) | **A** | TADIR `EVTB` / `EVTO`; `SAPRead(type="EVTB", name="<event_binding>")` |
...
| Flat file | **D** | `SAPSearch(searchType="source_code", query="OPEN DATASET")` |
