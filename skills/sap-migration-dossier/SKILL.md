...
3. Add migration signals:
   ...
   - Dependencies/where-used: `SAPContext(action="deps", type="<type>", name="<name>")` or `SAPNavigate(action="references", type="<type>", name="<name>", maxResults=1000)` (read `total`; `truncated=true` means the fan-in is degraded evidence)
