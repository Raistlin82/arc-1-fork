...
| [modernize-abap-to-btp-cap](modernize-abap-to-btp-cap/SKILL.md) | End-to-end orchestrator from a classic ABAP Z* package to a side-by-side BTP CAP application (Node.js + Fiori Elements V4 + CF): skeleton, clean-core gap analysis, then chains the two `modernize-abap-cap-*` sub-skills plus FE scaffold and MTA/XSUAA artifacts — all under a reversible staging dir | "Port this Z package to BTP CAP"; the `extract_to_side_by_side` arm of `sap-erp-clean-core-refactor` |
