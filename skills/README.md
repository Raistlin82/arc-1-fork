...
| [generate-cap-cds-test](generate-cap-cds-test/SKILL.md) | Generates and runs CAP model, service, authorization, event, and parity tests for a staged side-by-side target | Every CAP extraction before CF or Kyma packaging; blocks handoff while generated handlers remain TODO/501 stubs |
...
- `SAPContext(action="impact", type="DDLS", name="<ddls_name>")` for RAP/CDS reuse and "what breaks if I change this?" analysis
...
- `SAPWrite(action="batch_create", activateAtEnd=true, objects=[{type:"DDLS", name:"<name>", source:"<source>"}])` for atomic CDS-composition activation — replaces per-file + manual terminal activation in `migrate-segw-to-rap` Step 2 (ARC-1 v0.9.5+ / PR #270)
- `SAPTransport(action="history", type="<type>", name="<name>")` for current object lock/assignment status during later iterations (legacy name; candidates are assignment choices, not a complete history — use `SAPRead(type="VERSIONS", name="<name>", objectType="<type>")` for revisions)
...
| [sap-transport-review](sap-transport-review/SKILL.md) | Reviews what *changed* — in a transport or in your unactivated drafts — as per-object unified diffs (`SAPTransport(action="list", summary=true)` to scan, `SAPRead(action="diff", type="<type>", name="<name>", from="<from>", to="<to>")` to diff) plus risk flags and optional impact/ATC. The headless/whole-transport twin of Eclipse ADT 3.6's "Object Changes" | Pre-release/pre-activation gate, reviewing a transport (senior dev), "what have I changed since my last release?", change hand-off or audit |
...
| [sap-erp-clean-core-refactor](sap-erp-clean-core-refactor/SKILL.md) | Orchestrates a Clean Core refactor of a Z*/Y* package per logical unit: inventory + impact analysis, Level A–D/Unknown classification (via `sap-clean-core-atc`), an AEM-backed decision (SAP standard, Key User, embedded ABAP Cloud, BTP ABAP Environment, CF, Kyma, hybrid, wrapper, keep at Level B, retire), plan emission, then opt-in execution through ARC-1 with regression gates — DDIC changes run only when no system needs a database adjustment, the rest becomes an owned handoff | Refactoring D/C code to B/A (selective B→A escalation with `--push-to-a`); producing a documented, editable migration plan before touching the system |
| [modernize-abap-side-by-side-core](modernize-abap-side-by-side-core/SKILL.md) | Produces the mandatory architecture contract for a proposed BTP Level A target: released S/4 boundary, data ownership, consistency, identity, UI, runtime, rollback, and acceptance evidence | Before any CF or Kyma side-by-side implementation; decides whether CAP, persistence, UI, events, and runtime packaging are actually required |
...
| [modernize-abap-to-btp-cap](modernize-abap-to-btp-cap/SKILL.md) | End-to-end orchestrator from a classic ABAP Z* package to a side-by-side BTP CAP application: architecture contract, runtime-neutral staging, conditional schema/service/UI work, CAP tests, then exactly one CF or Kyma delivery branch | "Port this Z package to BTP CAP"; the `extract_to_side_by_side_cf`/`extract_to_side_by_side_kyma` arms of `sap-erp-clean-core-refactor` |
| [modernize-abap-cap-schema](modernize-abap-cap-schema/SKILL.md) | Translates Z-tables / DDIC structures into a CAP `db/schema.cds` data model | Step 3 of the `modernize-abap-to-btp-cap` chain; also standalone for schema-only ports |
| [modernize-abap-cap-service](modernize-abap-cap-service/SKILL.md) | Translates function modules / programs into CAP `srv/service.cds` + Node.js handler stubs | Step 4 of the `modernize-abap-to-btp-cap` chain; also standalone for service-layer ports |
| [scaffold-cap-fiori-elements](scaffold-cap-fiori-elements/SKILL.md) | Scaffolds a Fiori Elements V4 application from an approved CAP service and its live metadata, using discovered SAP Fiori MCP capabilities when available and a documented local fallback otherwise | The side-by-side contract selects `uiTarget=cap_fiori_elements` |
| [deploy-cap-to-kyma](deploy-cap-to-kyma/SKILL.md) | Prepares and verifies the official CAP Kyma/Helm delivery artifacts, image and service bindings, then deploys only with explicit cluster and deployment approval | The approved side-by-side runtime is Kyma; never for the CF branch |
