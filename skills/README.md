...
| [sap-erp-clean-core-refactor](sap-erp-clean-core-refactor/SKILL.md) | Orchestrates a full Clean Core refactor of a Z*/Y* package: inventory + impact analysis, Level A–D classification (via `sap-clean-core-atc`), per-object decision (rewrite-in-place / extract to BTP side-by-side / keep at Level B / remove), plan emission, opt-in execution with regression gates | Refactoring D/C code to B/A (selective B→A escalation with `--push-to-a`); producing a documented, editable migration plan before touching the system |
