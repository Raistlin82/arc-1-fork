...
- **Locally, over stdio**, from git at a pinned commit: `npx -y github:Raistlin82/arc-1-fork#<commit>`. npm runs `prepare` ([`scripts/prepare.mjs`](scripts/prepare.mjs)), which builds `dist/` in the git checkout, so no npm release is needed. The NOVA installer writes this entry into the project's `.mcp.json`, with the connection taken from environment variables (`SAP_URL`, `SAP_USER`, `SAP_PASSWORD`, `SAP_CLIENT`).
